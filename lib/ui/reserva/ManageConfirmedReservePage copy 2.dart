  ...
  Future<void> _uncheckReserva(int reservaId) async {
    ...
    if (confirm == true) {
      try {
        ...
        setState(() {
          // Se estava em serviço, move para confirmadas
          if (_currentTabIndex == 1) {
            var reserva = _reservasInService.firstWhere((r) => r.id == reservaId);
            _reservasInService.removeWhere((r) => r.id == reservaId);
            reserva.state = 'Not Confirmed';
            reserva.inService = 'No';
            _reservas.add(reserva);
          } else {
            // Se estava em confirmadas, apenas atualiza o estado
...
String _addPadding(String base64String) {
      ...
      builder: (context) {
        return AtendimentoForm(
          ...
          onProcessStart:
            ...
            // Atualiza o estado localmente
            setState(() {
              // Remove da lista de confirmadas
              var reserva = _reservas.firstWhere((r) => r.id == reservaId);
              _reservas.removeWhere((r) => r.id == reservaId);
              
              // Adiciona à lista de em serviço
              reserva.inService = 'Yes';
              _reservasInService.add(reserva);
  ...
  Widget _buildReservasList(List<Reserva> reservasToShow) {
    return _isLoading && reservasToShow.isEmpty
      ? const Center(child: CircularProgressIndicator())
      : reservasToShow.isEmpty
          ...
          : _isGridView
              ? GridView.builder(
                  ...
                  itemCount: reservasToShow.length + (_hasMore ? 1 : 0),
                  itemBuilder: (context, index) {
                    if (index >= reservasToShow.length) {
                      return _isLoading
                          ? const Center(child: CircularProgressIndicator())
                          : Container();
                    }
                    return _buildReservaCard(reservasToShow[index]);
                  },
                )
              : ListView.builder(
                  itemCount: reservasToShow.length + (_hasMore ? 1 : 0),
            ...
            children: [
              TabBar(
                ...
                tabs: [
                  Tab(
                    icon: Badge(
                      label: Text(_reservas.length.toString()),
                      isLabelVisible: _reservas.isNotEmpty,
                      child: const Text('Confirmed'),
                    ),
                  ),
                  Tab(
                    icon: Badge(
                      label: Text(_reservasInService.length.toString()),
                      isLabelVisible: _reservasInService.isNotEmpty,
