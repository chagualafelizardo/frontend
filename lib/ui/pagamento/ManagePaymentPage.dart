          ...
          return AlertDialog(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        FutureBuilder<List<Map<String, dynamic>>>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState == ConnectionState.waiting) {
                              return _buildInfoRow(Icons.person, 'Driver id:', 'Loading...');
                            } else if (snapshot.hasError) {
                              return _buildInfoRow(Icons.error, 'Driver id:', 'Error loading');
                            } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                              return _buildInfoRow(Icons.warning, 'Driver id:', 'No driver assigned');
                            } else {
                              // Get the first driver (assuming there might be multiple)
                              final driver = snapshot.data!.first;
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  // Get the driver ID first
                  final drivers = await _userAtendimentoAllocationService.getUserDetailsByAtendimentoId(atendimentoId);
                  if (drivers.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      _buildSnackBar('No driver assigned to this service', Colors.red),
                    );
                    return;
                  }
                  ...
                  try {
                    ...
                    final pagamento = Pagamento(
                      ...
                      userId: driverId, // Using driver ID instead of user ID
