...
void _filterAtendimentos() {
    ...
    setState(() {
      _filteredAtendimentos = _atendimentos.where((atendimento) {
        ...
        final atendimentoMatricula = atendimento.matricula?.toLowerCase() ?? '';

        return atendimentoDestino.contains(destino) &&
            atendimentoUser.contains(user) &&
...
void _showDriverDetailsDialog(int atendimentoId) async {
  try {
    ...
    // Agora exibimos os detalhes do motorista
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("ID do Motorista: ${motorista.id}"),
              Text("Nome: ${motorista.firstName} ${motorista.lastName}"),
              Text("Email: ${motorista.email ?? 'Não disponível'}"),
              Text("Telefone 1: ${motorista.phone1 ?? 'Não disponível'}"),
              Text("Telefone 2: ${motorista.phone2 ?? 'Não disponível'}"),
    ...
    // Lógica para lidar com erro, como mostrar uma mensagem para o usuário
  }
}
